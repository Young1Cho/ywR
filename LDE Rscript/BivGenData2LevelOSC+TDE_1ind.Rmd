...
a0 = runif(totalPersons,-2,2) #Initial condition means
for(a in a0){
xcov3_OU1 = simStateSpaceData(a0=a, P0=matrix(P0), Q=Psi,R=matrix(R),Phi=A_del,Lambda = 1,
...
#TVcov for y
ycov3=c()
a0 = runif(totalPersons,-2,2)
for(a in a0){
ycov3_OU1 = simStateSpaceData(a0=a, P0=matrix(P0), Q=Psi,R=matrix(R),Phi=A_del,Lambda = 1,
                  alpha=b_del,tau = rep(0,1), nt=maxT+1, np=1, ne=1, ny=1, nx=0, npad=0)
ycov3<-rbind(ycov3,ycov3_OU1$stateData)}
dim(ycov3)

cor.test(xcov3,ycov3)
plot(xcov3[1:1500],ycov3[1:1500])
...
DLOmodel <- function(t, prevState, parms) {
    ...
    cov3y <- ycov3[1+t+(id-1)*(maxT+1)] #cov3[1+t,id] #cov3 따로 다시 만들긴 해야함
...
for (id in 1:totalPersons) {
  ...
  if (Model=="Osc"){  
      ...
      zeta1= -0.04
      b31  =  0.3 + 0.1*cov1[id] + rnorm(1,0,sqrt(.04)) #+.2*cov1[id]
      coup1= -0.1
        
      eta2 = -0.5 #+ ifelse(etaresid>0.4,0.4,etaresid) #+.1*cov2[id]
      zeta2= -0.04
      b32  =  0.3 + 0.1*cov1[id] + rnorm(1,0,sqrt(.04)) #+.2*cov1[id]
      coup2= -0.1
...
plot(c(min(TimeSeq), max(TimeSeq)), c(-5,5),
  ...
  #lines(TimeSeq, withinFactor[id,1:(maxT+1)+1*(maxT+1)], type='b', lwd=1, col='grey') # TVcov_y
  lines(TimeSeq, withinFactor[id,1:(maxT+1)+2*(maxT+1)], type='b', lwd=1, col=id)    # x
  lines(TimeSeq, withinFactor[id,1:(maxT+1)+3*(maxT+1)], type='b', lwd=1, col=id+1)  # y
}
