...
# Sigma <- matrix(c(0, 0,
#                   0, 0),2,2);Sigma
# bvn <- data.frame(mvrnorm(totalPersons, mu=c(0,0), Sigma = Sigma)) # from MASS package
# names(bvn) <- c("mu1","mu2")
# plot(bvn)
# cor.test(bvn$mu1,bvn$mu2)
...
#TVcov for x
a01 = runif(totalPersons,-2,2) #Initial condition means
#mu1=bvn$mu1; b1 = a*mu1
#b_del1 = 1/A*(exp(A*deltaT)-1)*b1
xcov3=c()
for(i in 1:totalPersons){
  a0 <- a01[i]
  #b_del <- b_del1[i]
  xcov3_OU1 = simStateSpaceData(a0=a0, P0=matrix(P0), Q=Psi,R=matrix(R),Phi=A_del,Lambda = 1,
                  alpha=b_del,tau = rep(0,1), nt=(maxT+1), np=1, ne=1, ny=1, nx=0, npad=0)
  xcov3<-rbind(xcov3,xcov3_OU1$stateData) }
dim(xcov3);summary(xcov3)
  
#TVcov for y
a02 = runif(totalPersons,-2,2)
#mu2=bvn$mu2; b2 = a*mu2
#b_del2 = 1/A*(exp(A*deltaT)-1)*b2
ycov3=c()
for(i in 1:totalPersons){
  a0 <- a02[i]
  #b_del <- b_del2[i]
  ycov3_OU1 = simStateSpaceData(a0=a0, P0=matrix(P0), Q=Psi,R=matrix(R),Phi=A_del,Lambda = 1,
                  alpha=b_del,tau = rep(0,1), nt=(maxT+1), np=1, ne=1, ny=1, nx=0, npad=0)
  ycov3<-rbind(ycov3,ycov3_OU1$stateData)}
dim(ycov3);summary(ycov3)

cor.test(xcov3,ycov3)

par(mfrow=c(3,2))
for (id in sample(1:totalPersons,6)) {
#for (id in 1:6) {
plot(c(min(TimeSeq), max(TimeSeq)), c(-3,3),
     xlab="Time", ylab="Score", main = paste('ID =',id), type='n')
  lines(c(min(TimeSeq), max(TimeSeq)), c(-0, 0), type='l', lty=2, col=1)   # mu=0
  lines(TimeSeq, xcov3[(1:(maxT+1))+(maxT+1)*id], type='b', lwd=1, col=1)  # x
  lines(TimeSeq, ycov3[(1:(maxT+1))+(maxT+1)*id], type='b', lwd=1, col=2)  # y
}
...
# a=arima.sim(model=list(ar=0.97), n=10000,  sd=sqrt(0.1796))
# b=arima.sim(model=list(ar=0.97), n=10000,  sd=sqrt(0.1796))
# cor.test(a,b)
...
DLOmodel <- function(t, prevState, parms) {
    ...
    dx   <- prevState[2]
     y   <- prevState[3] # y[t]
    dy   <- prevState[4]
    cov3x <- xcov3[1+t+(id-1)*(maxT+1)] #cov3[1+t,id]
    cov3y <- ycov3[1+t+(id-1)*(maxT+1)] #cov3[1+t,id]

    with(as.list(parms), {
    #parms <- c(eta1, zeta1, b31, coup1, 
    #           eta2, zeta2, b32, coup2)
            dx  <- dx
            d2x <- parms[1]*x + parms[2]*dx + parms[3]*cov3x + parms[4]*y
            #d2x <- parms[1]*x + parms[2]*dx + parms[4]*y
            dy  <- dy
            d2y <- parms[5]*y + parms[6]*dy + parms[7]*cov3y + parms[8]*x
            #d2y <- parms[5]*y + parms[6]*dy + parms[8]*x
            res <-c(dx,d2x,dy,d2y)
            list(res)
    })
}
...
for (id in 1:totalPersons) {
  ...
  if (Model=="Osc"){  
      ...
      eta1 = -0.5 #+ ifelse(etaresid>0.4,0.4,etaresid) #+.1*cov2[id]
      zeta1= -0.04
      b31  =  0.3 + 0.1*cov1[id] + rnorm(1,0,sqrt(.04))
      #b31  =  0.3 + rnorm(1,0,sqrt(.04)) #+.2*cov1[id]
      coup1= -0.1
        
      eta2 = -0.5 #+ ifelse(etaresid>0.4,0.4,etaresid) #+.1*cov2[id]
      zeta2= -0.04
      b32  =  0.3 + 0.1*cov1[id] + rnorm(1,0,sqrt(.04))
      #b32  =  0.3 + rnorm(1,0,sqrt(.04))
      coup2= -0.1
    parms <- c(eta1, zeta1, b31, coup1, eta2, zeta2, b32, coup2)
    
    # xstart<-c(x=rnorm(1,mean=0,sd=1), dx=rnorm(1,mean=0,sd=1), cov3 = runif(1,-4,4)) # OU COV: initial value
    ...
    withinFactor[id,1:(maxT+1)]          <- xcov3[1:(maxT+1)+(id-1)*(maxT+1)]
    withinFactor[id,1:(maxT+1)+(maxT+1)] <- ycov3[1:(maxT+1)+(id-1)*(maxT+1)]
    
    xstart <- c(x=rnorm(1,0,1), dx=rnorm(1,0,1),y=rnorm(1,0,1),dy=rnorm(1,0,1)) # initial values
    out1 <- as.data.frame(lsoda(xstart, TimeSeq, DLOmodel, parms))
    withinFactor[id,1:(maxT+1)+2*(maxT+1)] <- out1$x #x: repeated measure/ID (wide)
    withinFactor[id,1:(maxT+1)+3*(maxT+1)] <- out1$y #y: repeated measure/ID (wide)
...
colnames(withinFactor) = c(paste0("cov3x",0:maxT),paste0("cov3y",0:maxT),
                           paste0("x",0:maxT),paste0("y",0:maxT),
                           "ID","cov1","cov2",'intcov')
...
plot(c(min(TimeSeq), max(TimeSeq)), c(-5,5),
     xlab="Time", ylab="Score", main = paste('ID =',id,' coup1=',coup1), type='n')
  lines(c(min(TimeSeq), max(TimeSeq)), c(-0, 0), type='l', lty=2, col=1) # setpoint
  
  lines(TimeSeq, withinFactor[id,1:(maxT+1)+2*(maxT+1)], type='b', lwd=1, col=1)     # x
  #lines(TimeSeq, withinFactor[id,1:(maxT+1)+0*(maxT+1)], type='b', lwd=2, col='grey') # TVcov_x
  lines(TimeSeq, withinFactor[id,1:(maxT+1)+3*(maxT+1)], type='b', lwd=1, col=2)     # y
  #lines(TimeSeq, withinFactor[id,1:(maxT+1)+1*(maxT+1)], type='b', lwd=1, col='pink') # TVcov_y
}
...
xdataMatrix2 = matrix(NA,totalPersons*(maxT+1),ny)
ydataMatrix = matrix(NA,totalPersons*(maxT+1),ny) 
ydataMatrix2 = matrix(NA,totalPersons*(maxT+1),ny)
...
for (t in 1:(maxT+1)){
x1it[id,] = withinFactorLoadings %*% withinFactor[id,2*(maxT+1)+t] + chol(R)%*%rnorm(ny,0,1)
...
xdataMatrix2[t+(id-1)*(maxT+1),] = x1it[id,]

ydataMatrix[t+(id-1)*(maxT+1),] = y2i[id,] + y1it[id,]
ydataMatrix2[t+(id-1)*(maxT+1),] = y1it[id,]
}}
```
...
```{r}
setwd("C:/Users/yw110/OneDrive/PennState/ywR/Data")
getwd()
```
...
for (id in 1:totalPersons){ 
  ...
  long[(1:(maxT+1))+(id-1)*(maxT+1),4] = withinFactor[id, 1:(maxT+1)]               #xTVcov
  
  long[(1:(maxT+1))+(id-1)*(maxT+1),5] = ydataMatrix[(1:(maxT+1))+(id-1)*(maxT+1),] #y1
  long[(1:(maxT+1))+(id-1)*(maxT+1),6] = withinFactor[id, 1:(maxT+1)+(maxT+1)]      #yTVcov
...
colnames(long) = c("ID","Time", paste0("x",1:ny),'xTVcov',paste0("y",1:ny),'yTVcov')
...
colnames(embedAll) = c("ID",
                       paste0("x1lag",(embedD-1):0), paste0("xTVlag",(embedD-1):0),
                       paste0("y1lag",(embedD-1):0), paste0("yTVlag",(embedD-1):0))
colnames(embedAll)
...
head(embedAll)
Model="Osc"
write.table(round(embedAll,5),
            file=paste0(Model,Type,"_TDE",embedD,"_",TVcovType,"_n",totalPersons,'_t',maxT+1,".txt"),
